...
class WSAL_Sensors_WPFormsSensor extends WSAL_AbstractSensor {
    ...
    public function HookEvents() {
        add_action( 'pre_post_update', [ $this, 'get_before_post_edit_data' ], 10, 2 );
        add_action( 'save_post', [ $this, 'event_form_saved' ], 10, 3 );
        add_action( 'delete_post', [ $this, 'event_form_deleted' ], 10, 1 );
        add_action( 'wpforms_pre_delete', [ $this, 'event_entry_deleted' ], 10, 1 );
        add_action( 'wpforms_pro_admin_entries_edit_submit_completed', [ $this, 'event_entry_modified' ], 5, 4 );
        add_action( 'updated_option', [ $this, 'event_settings_updated' ], 10, 3 );
        add_action( 'added_option', [ $this, 'event_added_option' ], 10, 2 );
        add_action( 'wpforms_plugin_activated', [ $this, 'addon_plugin_activated' ], 10, 1 );
        add_action( 'wpforms_plugin_deactivated', [ $this, 'addon_plugin_deactivated' ], 10, 1 );
        add_action( 'wpforms_plugin_installed', [ $this, 'addon_plugin_installed' ], 10, 1 );
    }
    ...
    public function event_form_saved( $post_id, $post, $update ) {
        ...
        if ( ! isset( $this->_old_post->post_title ) && ! $update && ! preg_match( '/\s\(ID #[0-9].*?\)/', $form->post_title ) && 'wpforms' === $post->post_type ) {
            ...
            $this->plugin->alerts->trigger_event_if( $alert_code, $variables, [ $this, 'check_if_duplicate' ] );
            ...
            if ( isset( $post->post_status ) && 'auto-draft' !== $post->post_status ) {
                ...
                $this->plugin->alerts->trigger_event_if( $alert_code, $variables, [ $this, 'must_not_be_new_form' ] );
                $has_alert_triggered = true;
            }
        }
        ...
        if ( preg_match( '/\s\(ID #[0-9].*?\)/', $form->post_title ) && 'wpforms' === $form->post_type ) {
            ...
            if ( $form->post_date_gmt === $form->post_modified_gmt ) {
                ...
                if ( isset( $old_form_content->id ) ) {
                    ...
                    $has_alert_triggered = true;
                    remove_action( 'save_post', [ $this, 'event_form_saved' ], 10, 3 );
                }
            }
        }
        ...
        if ( 'wpforms' === $form->post_type && isset( $this->_old_post ) ) {
            ...
            if ( isset( $post->post_status ) && 'auto-draft' !== $post->post_status ) {
                ...
                if ( isset( $form_content->fields ) && isset( $old_form_content->fields ) && serialize( $form_content->fields ) !== serialize( $old_form_content->fields ) ) {
                    ...
                    if ( $changed_items && ! self::was_triggered_recently( 5500 ) ) {
                        $alert_code = 5501;
    ...
    public function event_entry_deleted( $row_id ) {
        ...
        $this->plugin->alerts->trigger_event( $alert_code, $variables );
        remove_action( 'wpforms_pre_delete', [ $this, 'event_entry_deleted' ], 10, 1 );
    }
    ...
    public function event_entry_modified( $form_data, $response, $updated_fields, $entry ) {
        ...
        foreach ( $updated_fields as $updated_field ) {
            ...
            $modified_value = [ array_search( $updated_field['name'], array_column( $fields, 'name', 'value' ) ) ];
