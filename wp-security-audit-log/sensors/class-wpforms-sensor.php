...
if ( ! class_exists( '\WSAL\Plugin_Sensors\WPForms_Sensor' ) ) {
	/**
	...
	class WPForms_Sensor {
		...
		public static function event_form_saved( $post_id, $post, $update ) {
			...
			if ( ! isset( self::$_old_post->post_title ) && ! $update && ! preg_match( '/\s\(ID #[0-9].*?\)/', $form->post_title ) && 'wpforms' === $post->post_type ) {
				$alert_code  = 5500;
			...
			} elseif ( isset( self::$_old_post->post_title ) && self::$_old_post->post_title !== $post->post_title && 'wpforms' === $post->post_type && $update ) {
				...
				if ( isset( $post->post_status ) && 'auto-draft' !== $post->post_status ) {
					...
					$variables = array(
						'EventType'      => 'renamed',
						'old_form_name'  => sanitize_text_field( self::$_old_post->post_title ),
			...
			if ( preg_match( '/\s\(ID #[0-9].*?\)/', $form->post_title ) && 'wpforms' === $form->post_type ) {
				...
				if ( $form->post_date_gmt === $form->post_modified_gmt ) {
					...
					$old_form_content = self::json_decode_encode( self::$_old_post->post_content );
					$editor_link      = self::create_form_post_editor_link( $post_id );

					if ( isset( $old_form_content->id ) ) {
						$variables = array(
							'OldPostTitle'             => sanitize_text_field( self::$_old_post->post_title ),
			...
			if ( 'wpforms' === $form->post_type && isset( self::$_old_post ) ) {
				if ( isset( $post->post_status ) && 'auto-draft' !== $post->post_status ) {
			...
			if ( 'wpforms' === $form->post_type && isset( self::$_old_post ) && $update && ! Alert_Manager::was_triggered_recently( 5500 ) ) {
				// Checking to ensure this is not a draft or fresh form.
				if ( isset( $post->post_status ) && 'auto-draft' !== $post->post_status ) {
					$form_content     = self::json_decode_encode( $form->post_content );
					$old_form_content = self::json_decode_encode( self::$_old_post->post_content );
			...
			if ( ! $has_alert_triggered && 'wpforms' === $form->post_type && isset( self::$_old_post ) && ! $update && ! Alert_Manager::was_triggered_recently( 5500 ) ) {
				if ( isset( $post->post_status ) && 'auto-draft' !== $post->post_status ) {
					$alert_code       = 5500;
					$form_content     = self::json_decode_encode( $form->post_content );
					$old_form_content = self::json_decode_encode( self::$_old_post->post_content );
